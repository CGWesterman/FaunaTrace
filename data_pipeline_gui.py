...
try:
    from data_pipeline import load_data, write_clean_data, CSV_REL, OUT_REL
except Exception:
...
class DataPipelineGUI(tk.Tk):
    def __init__(self):
        ...
        self._build_ui()
        # Try to load default data on startup so the UI isn't blank
        try:
            self.log_msg("Starting GUI — attempting to load default CSV...")
            self.load_default()
        except Exception as e:
            # load_default already logs errors; ensure the GUI still shows
            self.log_msg("Startup load failed:", e)
    ...
    def _build_ui(self):
        ...
        ttk.Button(top, text="Add Column", command=self.add_column_prompt).pack(side=tk.LEFT, padx=6)
    ...
    def add_column_prompt(self):
        """Ask user for a column name and an initial value/expression, then add it."""
        if self.df is None:
            self.log_msg("No data loaded")
            return
        
        name = simpledialog.askstring("New Column", "Column name:", parent=self)
        if not name:
            return
            
        if name in self.df.columns:
            overwrite = tk.messagebox.askyesno("Overwrite Column", f"Column '{name}' already exists. Overwrite?")
            if not overwrite:
                self.log_msg("Add column cancelled (exists):", name)
                return
            
        # initial value prompt:
        fill = simpledialog.askstring(
            "Initial Value",
            "Initial value (leave blank for empty). Prefix with '=' to evaluate pandas expression, e.g. '=longitude + latitude':",
            parent=self
        )

        self.add_column(name, fill)

    def add_column(self, name: str, fill: str | None):
        """
        Add a column to self.df.
        Behavior:
           - If fill is None => make empty string column
           - If fill starts with '=' => evaluate as pandas expression (df.eval)
           - Else try to parse Python literal (numbers, lists) with ast.iteral_eval, fallback to string
        """
        try:
            if fill is None:
                # empty string column (you can change to pd.NA if preferred)
                self.df[name] = ""
            else:
                fill = str(fill).strip()
                if fill == "":
                    ...
                    expr = fill[1:]
                    # Evaluate expression using pandas.eval on the DataFrame
                    # This will allow expressions that reference other columns
                    self.df[name] = self.df.eval(expr)
                else:
                    # try to interpret as literal (number, list, dict); fall back to string
                    try:
                        val = ast.literal_eval(fill)
                    except Exception:
                        val = fill
                    # If val is scalar, broadcast; if it's sequence and length matches, assign directly
                    if len(val) == len(self.df):
                        self.df[name] = val
                    else:
                        # fallback: convert sequence to string and broadcast
                        self.df[name] = str(val)
            
            self._populate_columns()
            self.preview()
            self.log_msg(f"Added column: {name}")
        except Exception as e:
            self.log_msg("Error adding column:", e)
            self.log_msg(traceback.format_exc())
